...
def _replace_submodules(root_module, predicate, func):
    ...
    for *parent, k in bn_list:
        ...
        if isinstance(parent_module, nn.Sequential):
            tgt_module = func(parent_module[int(k)])
            parent_module[int(k)] = tgt_module
        else:
            tgt_module = func(getattr(parent_module, k))
...
def _replace_bn_with_gn(root_module, features_per_group=16):
...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        backbones = _replace_bn_with_gn(backbones) # TODO
