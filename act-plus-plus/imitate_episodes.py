...
def log_to_tensorboard(writer, metrics, step):
    """Log scalar metrics to TensorBoard, handling torch tensors safely."""
...
def main(args):
    ...
    state_dim = args['state_dim'] if args['state_dim'] is not None else task_config.get('state_dim', 14)
    action_dim = args['action_dim'] if args['action_dim'] is not None else task_config.get('action_dim', 16)
    ...
    if base_action_dim < 0 or base_action_dim > action_dim:
        raise ValueError(f'base_action_dim ({base_action_dim}) must be between 0 and action_dim ({action_dim})')
    ...
    if not is_eval:
        tb_root_dir = '/tensorboard_log'
        tb_dir = os.path.join(tb_root_dir, expr_name)
        os.makedirs(tb_dir, exist_ok=True)
        writer = SummaryWriter(log_dir=tb_dir)
        writer.add_text('experiment/name', expr_name, 0)
        writer.add_text('experiment/config', repr(config), 0)
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    if real_robot:
        ...
        env = make_real_env(init_node=True, setup_robots=True, setup_base=setup_base)
        env_max_reward = 0
    else:
    ...
    if real_robot and base_action_dim > 0:
        BASE_DELAY = 13
            ...
            for t in range(max_timesteps):
                ...
                if config['policy_class'] == "ACT":
                    if t % query_frequency == 0:
                        ...
                        if real_robot and base_action_dim > 0:
                            all_actions = torch.cat([all_actions[:, :-BASE_DELAY, :-base_action_dim], all_actions[:, BASE_DELAY:, -base_action_dim:]], dim=2)
                    if temporal_agg:
                ...
                if base_action_dim > 0:
                    target_qpos = action[:-base_action_dim]
...
def should_run_rollout_eval(config):
    if config.get('skip_rollout_eval', False):
        return False
    if not config['real_robot']:
        return False
    return config.get('eval_on_real_robot', False)
...
def train_bc(train_dataloader, val_dataloader, config, writer):
    ...
    for step in tqdm(range(num_steps+1)):
        ...
        if step % validate_every == 0:
            ...
            log_to_tensorboard(writer, validation_summary, step)
        ...
        if (step > 0) and (step % eval_every == 0):
            ...
            if not should_run_rollout_eval(config):
                print('Skipping rollout evaluation on this machine (use --eval_on_real_robot to enable it, or --no_rollout_eval to force skip).')
            else:
                success, _ = eval_bc(config, ckpt_name, save_episode=True, num_rollouts=10)
                log_to_tensorboard(writer, {'success': success}, step)
        ...
        loss.backward()
        optimizer.step()
        log_to_tensorboard(writer, forward_dict, step) # not great, make training 1-2% slower
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_on_real_robot', action='store_true', default=False)
    parser.add_argument('--no_rollout_eval', action='store_true', default=False)
